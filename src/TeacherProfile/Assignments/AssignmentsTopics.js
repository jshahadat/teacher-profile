...
const AssignmentsTopics = () => {
    ...
    const randomAssignmentTopics = randomArrayValue(assignmentTopics, 5);

    useEffect(() => {
        fetch(`https://assignment-twelfth-server.vercel.app/assigmenttopics?courseCode=${courseCode}`, {
            // headers: {
            //     authorization: `Bearer ${localStorage.getItem('accessToken')}`
            // }
        })
            .then(res => res.json())
            .then(data => setAssignmentTopics(data))
    }, [])
            ...
            <div className='w-80 mx-auto mt-12'>
                {
                    ...
                    randomAssignmentTopics.map((randomAssignmentTopic, i) => <AssignmentTopic
                        key={randomAssignmentTopic._id}
                        randomAssignmentTopic={randomAssignmentTopic}
                        i={i}
                    ></AssignmentTopic>)
                }

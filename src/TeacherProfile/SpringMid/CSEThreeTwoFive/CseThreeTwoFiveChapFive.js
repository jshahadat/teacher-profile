...
import CseThreeTwoFiveChapFiveQuestion from './CseThreeTwoFiveChapFiveQuestion';

const CseThreeTwoFiveChapFive = () => {
    ...
    useEffect(() => {
        fetch("https://assignment-twelfth-server.vercel.app/threetwofivechapterfive?chapCode=5CSE-325Spring", {
            // headers: {
            //     authorization: `Bearer ${localStorage.getItem('accessToken')}`
            // }
        })
            .then(res => res.json())
            .then(data => setChapterFives(data))
    }, [])
    return (
        <div>
                                ...
                                <div>
                                    {
                                        ...
                                        chapterFiveRandomQuestions.map(chapterFiveQuestion => <CseThreeTwoFiveChapFiveQuestion
                                            key={chapterFiveQuestion._id}
                                            chapterFiveQuestion={chapterFiveQuestion}
                                        ></CseThreeTwoFiveChapFiveQuestion>)
                                    }
...
export default CseThreeTwoFiveChapFive;
...
import CseThreeTwoFiveChapTwoQuestion from './CseThreeTwoFiveChapTwoQuestion';

const CseThreeTwoFiveChapTwo = () => {
    ...
    const [chapterTwoes, setChapterTwoes] = useState([])
    ...
    const chapterTwoRandomQuestions = randomArrayValue(chapterTwoes, 5);

    useEffect(() => {
        fetch("https://assignment-twelfth-server.vercel.app/threetwofivechaptertwo?chapCode=2CSE-325Spring", {
            // headers: {
            //     authorization: `Bearer ${localStorage.getItem('accessToken')}`
            // }
        })
            .then(res => res.json())
            .then(data => setChapterTwoes(data))
    }, [])
    return (
        <div>
                                ...
                                <div>
                                    {
                                        ...
                                        chapterTwoRandomQuestions.map(chapterTwoQuestion => <CseThreeTwoFiveChapTwoQuestion
                                            key={chapterTwoQuestion._id}
                                            chapterTwoQuestion={chapterTwoQuestion}
                                        ></CseThreeTwoFiveChapTwoQuestion>)
                                    }
...
export default CseThreeTwoFiveChapTwo;